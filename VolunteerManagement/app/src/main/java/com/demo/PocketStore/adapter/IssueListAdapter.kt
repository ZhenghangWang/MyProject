...
class IssueListAdapter(private val list: List<IssueData>, private val mContext: Context) :
    BaseAdapter() {
    private var listview: ListView? = null
    private var mUserDataManager: IssueDataManager? = null
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        holder.tv_name!!.text =
            mDataManager!!.getStringByColumnName("name", meal.organization_id) + ""
        holder.tv_email!!.text = meal.content + ""
    ...
    fun modifyDataBase(mUser: IssueData) {
        if (mUserDataManager == null) {
            mUserDataManager = IssueDataManager(mContext)
            mUserDataManager!!.openDataBase()
        }
        mUserDataManager!!.openDataBase()
        val flag = mUserDataManager!!.updateDataById(mUser, mUser.id)
        if (flag) {
            Toast.makeText(mContext, "Resolve successfully", Toast.LENGTH_SHORT).show()
        } else {
        }
    }
